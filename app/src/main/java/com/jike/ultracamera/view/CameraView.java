...
/*
  ...
  public void openCamera(int width, int height, String camId) {
    this.camId = camId;
    ...
    setAspectRatio(CameraResolution.getPicSize().getHeight(), CameraResolution.getPicSize().getWidth());
  ...
  public class C2Helper{
    ...
    private void setUpCameraOutputs() {
      ...
      try {
        ...
        mCameraCharacteristics = manager.getCameraCharacteristics(camId);
        mCameraId = camId;
        c2helper.getCameraArgs();
      } catch (CameraAccessException e) {
        e.printStackTrace();
    ...
    private void createCameraPreviewSession() {
      try {
        ...
        texture.setDefaultBufferSize(CameraResolution.getPicSize().getWidth(), CameraResolution.getPicSize().getHeight());
        ...
        switch (CamMode.mode){
          case NORMAL:
            ...
            for(int i = 0; i<5;i++){
              ...
              if(i==0) builder.set(CaptureRequest.CONTROL_AE_EXPOSURE_COMPENSATION, 0);
              if(i==1) builder.set(CaptureRequest.CONTROL_AE_EXPOSURE_COMPENSATION, 10);
              if(i==2) builder.set(CaptureRequest.CONTROL_AE_EXPOSURE_COMPENSATION, -10);
              if(i==3) builder.set(CaptureRequest.CONTROL_AE_EXPOSURE_COMPENSATION, 20);
              if(i==4) builder.set(CaptureRequest.CONTROL_AE_EXPOSURE_COMPENSATION, -20);
          ...
          case PIX_FUSION:
            if(!ProReader.fusionKey) {
              ...
              for (int i = 0; i < ProReader.FUSION_COUNT; i++) {
                ...
                builderSuperRes.set(CaptureRequest.SENSOR_EXPOSURE_TIME, (long) (CameraParameter.exposureTime * (int) (CameraParameter.iso / 50f)*CameraParameter.isoBoost/100f) + (CamSetting.isNightOpened ? CameraParameter.ONE_SECOND * 2 : 0));
            ...
            *//*if(!ProReader.nightKey) {
              ...
              for (int i = 0; i < ProReader.NIGHT_COUNT; i++) {
                ...
                *//*if(i==0) {
                  ...
                  builderSuperNight.set(CaptureRequest.SENSOR_EXPOSURE_TIME, CameraParameter.exposureTime / 2);
                  builderSuperNight.set(CaptureRequest.SENSOR_SENSITIVITY, 200);
                } else {*//*
                if(!CamSetting.isDenoiseOpened) {
                  ...
                  builderSuperNight.set(CaptureRequest.SENSOR_SENSITIVITY, (int) ((CameraParameter.iso / (CameraParameter.ONE_SECOND_DIV_8 / CameraParameter.exposureTime)) * CameraParameter.isoBoost / 100f) / 2);


                *//*if(i==ProReader.NIGHT_COUNT-2) {
                  builderSuperNight.set(CaptureRequest.NOISE_REDUCTION_MODE, CaptureRequest.NOISE_REDUCTION_MODE_HIGH_QUALITY);
                  builderSuperNight.set(CaptureRequest.CONTROL_POST_RAW_SENSITIVITY_BOOST, 100);
                  builderSuperNight.set(CaptureRequest.SENSOR_EXPOSURE_TIME, (long) (CameraParameter.exposureTime ));
                  builderSuperNight.set(CaptureRequest.SENSOR_SENSITIVITY, 300);
                }*//*

                *//*if(i>=7) {
                  builderSuperNight.set(CaptureRequest.NOISE_REDUCTION_MODE, CaptureRequest.NOISE_REDUCTION_MODE_FAST);
                  builderSuperNight.set(CaptureRequest.CONTROL_POST_RAW_SENSITIVITY_BOOST, 100);
                  builderSuperNight.set(CaptureRequest.SENSOR_EXPOSURE_TIME, (long) (CameraParameter.exposureTime * (int) (CameraParameter.iso / 50f)*CameraParameter.isoBoost/100f));
          ...
          case SUPER_RES:
            if(!ProReader.resKey) {
              ...
              for (int i = 0; i < ProReader.RES_COUNT; i++) {
                ...
                builder1.set(CaptureRequest.SENSOR_SENSITIVITY, (int) ((int) (CameraParameter.iso / (CameraParameter.ONE_SECOND / 4.0f / CameraParameter.exposureTime)) * CameraParameter.isoBoost / 100.0f));
    ...
    public void getCameraArgs(){
      ...
      CameraResolution.setPicSizes(Arrays.asList(map.getOutputSizes(ImageFormat.JPEG)));
      CameraResolution.setVideoSizes(Arrays.asList(map.getOutputSizes(MediaRecorder.class)));
    ...
    private void faceDetect(TotalCaptureResult result){
      ...
      if(faces!=null) {
        ...
        for (Face face : faces) {
          ...
          if(point!=null) {
            point.x = (int) (screenH * ((float) point.x / CameraResolution.getPicSize().getWidth()));
            point.y = (int) (screenW - screenW * ((float) point.y / CameraResolution.getPicSize().getHeight()));
            points[i] = point;
          }
          rects[i]=rect;
          i++;
